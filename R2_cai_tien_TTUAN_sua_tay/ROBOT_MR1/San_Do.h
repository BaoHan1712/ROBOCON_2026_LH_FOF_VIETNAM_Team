...
void Xuat_Phat_Lay_Vu_Khi_Do(int vu_khi, int goc_ve, int vitri) {					
				...
				for(i=0;i<250;i++)
				{
					...
					{	
						Bam_thanh_laser_trai(45,50,0,vu_khi  ,2,-120,30);
						vTaskDelay(1); 
						if(!wantExit())	break;
					}
				}
				...
				for(i=0;i<250;i++)
				{
					...
					{	
						...
						Bam_thanh_laser_trai(35,40,0,vu_khi ,1,-100,15);
						vTaskDelay(1); 
						if(!wantExit())	break;
					}
				}
				...
				for(i=0;i<250;i++)
				{
					...
					{	
						...
						Bam_thanh_laser_trai(26,0,0,vu_khi ,1,-100,15);
						vTaskDelay(1); 
						if(!wantExit())	break;
					}
				}
				...
				for(i=0;i<250;i++)
				{
					...
					{	
						...
						Bam_thanh_laser_trai(22,0,0,vu_khi ,1,-100,15);
						vTaskDelay(1); 
						if(!wantExit())	break;
					}
				}
				...
				for(i=0;i<100;i++)
				{
					...
					{			
						...
						Bam_thanh_laser_trai(12,0,0,vu_khi,1,-50,15);
						vTaskDelay(1); 
						if(!wantExit())	break;
					}
				}
				...
				for(i=0;i<500;i++)
				{
					...
					{	
						if	(CB_tay_kep_vk_T == 1) robotRun(880,10);
						else if 	(CB_tay_kep_vk_P == 1) robotRun(-880,10);
						else		robotStop(0);				
						vTaskDelay (1);
						if(!wantExit())	break;
					}
...
void Xoay_dau_truoc_cua_do_1(void) {
	...
	su_dung_chan(480);
	run_encoder(21000, 1610, -900, 70, 10, 6000, 100);
...
///// chay ngang	check
	for(i=0;i<550;i++)	         //// check con 1 co dung day kh
		{	
			...
			{
				Bam_laser_sau(55,0,-900,135, 3, 150, 15);
				vTaskDelay(1); 
				if(!wantExit())	break;
			}
		}	
...
////////======================================		
		...
		{	
			...
			{
				Bam_laser_sau(50,0,-900,135, 3, 150, 15);
				vTaskDelay(1); 
				if(!wantExit())	break;
			}
		}
...
void tudonghoantoan_san_do(void) {
	...
	// cho rb1 lay vk xong thì vuot rung
		...
		if (hoan_thanh_chay_rung == 1 ) DatKFS_do(160),hoan_thanh_chay_rung = 2 ;
	/// ******* //////
