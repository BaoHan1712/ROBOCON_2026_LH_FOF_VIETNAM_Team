...
/////////// uart6 //////
void DMA2_Stream1_IRQHandler(void)// ngat DMA uart6 RX
{
...
//}  
extern uint8_t rx_buf[10], GP_BTN [15];
void UART_DMA_Reset();
//////////////////////////////
void DMA1_Stream1_IRQHandler(void)
{
    if (DMA_GetITStatus(DMA1_Stream1, DMA_IT_TCIF1))
    {
        ...
        // kiá»ƒm tra checksum
        uint8_t sum = 0;
        for(int i=0;i<9;i++) sum += rx_buf[i];

        if(sum == rx_buf[9])
        {
            memcpy(GP_BTN, rx_buf, 10);
            gp_update_uart();
        }
        else
        {
        ...
        // reset DMA Ä‘á»ƒ nháº­n frame má»›i
...
void DMA1_Stream2_IRQHandler(void)
{
    if(DMA_GetITStatus(DMA1_Stream2, DMA_IT_TCIF2))
    {
        ...
        // Reset l?i con tr? và kích thu?c buffer
        DMA_SetCurrDataCounter(DMA1_Stream2, 8);
