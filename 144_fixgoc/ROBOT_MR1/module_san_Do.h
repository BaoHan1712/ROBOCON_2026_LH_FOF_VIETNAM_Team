...
void chinh_lai_vi_tri_laser_phai_custom_do(int vitri, int gia_tri_lap, int max_toc)
{
	...
	int dieuchinh;
		...
		while(abs(lazePhaiValue - vitri) > 1)	{
			dieuchinh = lazePhaiValue - vitri;
			speed = custom_smooth_speed(dieuchinh, 5, max_toc);
	...
	if ( TinHieu_ChuanBi_GapThang == 1 ) {
				...
				for(i=0;i<1050;i++)	
					...
					if (chay_ngang_qua_o3_do == 0) {
							chinh_lai_vi_tri_laser_phai_do(vi_tri_cua_rung_do_2_lazer_phai_tay_41 - 15, 1000);
			}
			else if (chay_ngang_qua_o3_do == 1) {
						chinh_lai_vi_tri_laser_phai_custom_do(vi_tri_cua_rung_do_1_lazer_phai_tay_41 - 15, 1000, 60);
						}
				}
	...
	kiem_tra_huong_quay_tay_gap_phia_truoc();
		...
		if ( TinHieu_ChuanBi_GapThang == 1 ) {
				...
				if (chay_ngang_qua_o1_do == 0) {
						...
						chinh_lai_vi_tri_laser_phai_custom_do(vi_tri_cua_rung_do_3_lazer_phai_tay_41 - 15, 1000,  60);
						}
				}
...
void di_tren_buc_khong_thanh_2_ben_do(void) {
		if (da_lay_tay1 == 0 && da_lay_tay2 == 0 && da_lay_tay3 == 0 && da_lay_tay4 == 0) {
				...
				if(CB_ROI_3  == 0 && CB_ROI_1 == 0)	robotRunAngle(1150,5,-900,0.7); // di vosi goc xéo phai
				else																robotRunAngle(900,5,-900,0.7); // di thang
				vTaskDelay(1);
				if(!wantExit()) break;
					}
				// chay toi roi thi di ngang qua phai
				robotRunAngle(1800,5,-900,0.7);
				while (CB_ROI_3  == 0 )	 {
					vTaskDelay(1);
					if(!wantExit()) break;
					}
		...
		//fix1
				...
				while(CB_vtri_leo == 0 || CB_Ha_Sau == 0) {
				if(CB_ROI_3  == 0 && CB_ROI_1 == 0)	robotRunAngle(1150,5,-900,0.7); // di xeo phai
				else																robotRunAngle(900,5,-900,0.7);  // di thang 
				vTaskDelay(1);
				if(!wantExit()) break;
					}
...
//fix2
		// quay mam sang tay 2
				...
				while(CB_vtri_leo == 0 ||  CB_Ha_Sau == 0) {
				if(CB_ROI_2  == 0 && CB_ROI_4 == 0)	robotRunAngle(650,5,-900,0.7); //di xeo qua trai
				else																robotRunAngle(900,5,-900,0.7); //di thang
				vTaskDelay(1);
				if(!wantExit()) break;
							}
				// chay toi roi thi di ngang qua trai
				robotRunAngle(0,5,-900,0.7);
				while (CB_ROI_2  == 0 )	 {
					vTaskDelay(1);
					if(!wantExit()) break;
						}
		...
		// quay mam sang tay 3
				...
				while(CB_vtri_leo == 0 ||  CB_Ha_Sau == 0) {
				if(CB_ROI_2  == 0 && CB_ROI_4 == 0)	robotRunAngle(650,5,-900,0.7);
				else																robotRunAngle(900,5,-900,0.7);
				vTaskDelay(1);
				if(!wantExit()) break;
						}
				// chay toi roi thi di ngang qua trai
...
void Leo_bac_400_co_thanh_do(void) {
		...
		for(i=0;i<1550;i++)
				{
					while(CB_Cap_Thanh_Sau == 0 || (abs(bientrochansauValue - target_chan_sau) > 7))	
					{	
						vTaskDelay (2);
						if(!wantExit())	break;
					}
...
void chuan_bi_gap_thang_do_9 (void) {
		...
		su_dung_chan(265);
			...
			if (da_lay_tay1 == 0 && da_lay_tay2 == 0 && da_lay_tay3 == 0 && da_lay_tay4 == 0) {
				while( CB_Ha_Sau == 0 || (abs(bientrochansauValue - target_chan_sau) > 10)) 
					{	
					if(lazeTraiValue > vitri_tay41)			robotRunAngle(1150,20,-900,0.7);
				...
				while(CB_Ha_Sau == 0 || (abs(bientrochansauValue - target_chan_sau) > 10)) 
					{	
					if(lazeTraiValue > vitri_tay41)			robotRunAngle(1150,5,-900,0.7);
					else																robotRunAngle(900,5,-900,0.7);
					vTaskDelay(1);
					if(!wantExit()) break; }
				}
					...
					while(CB_Ha_Sau == 0 || (abs(bientrochansauValue - target_chan_sau) > 10)) {
				if(CB_ROI_2  == 0 && CB_ROI_4 == 0)	robotRunAngle(650,18,-900,0.7);
				else																robotRunAngle(900,15,-900,0.7);
				vTaskDelay(1);
				if(!wantExit()) break;
					}
...
//			KT_Ha_Tay = 1;
		...
		if (da_lay_tay1 == 0 && da_lay_tay2 == 0 && da_lay_tay3 == 0 && da_lay_tay4 == 0) {
					...
					chinh_lai_vi_tri_laser_trai(vitri_tay41, 1000);	
						...
						while( CB_Cap_Thanh_Sau == 0 ||  (abs(bientrochansauValue - target_chan_sau) > 10))	
						{	
						if(lazeTraiValue < vitri_tay23)			robotRunAngle(650,5,-900,0.7);
						else																robotRunAngle(900,5,-900,0.7);
						vTaskDelay(1);
						if(!wantExit()) break;
									}
...
//		KT_Ha_Tay = 1;
	// chinh lai vi tri cho tay gap
		if (da_lay_tay1 == 0 && da_lay_tay2 == 0 && da_lay_tay3 == 0 && da_lay_tay4 == 0) {
				while( CB_Cap_Thanh_Sau == 0 || (abs(bientrochansauValue - target_chan_sau) > 10) )	{
						if(CB_ROI_1  == 0 && CB_ROI_3 == 0)	robotRunAngle(1150,5,-900,0.7);
						else																robotRunAngle(900,5,-900,0.7);
						vTaskDelay(1);
						if(!wantExit()) break;
						}
			...
			for(i=0;i<1550;i++) {
						while( CB_Cap_Thanh_Sau == 0 ||(abs(bientrochansauValue - target_chan_sau) > 10))	
						{	
						if(lazePhaiValue > vitri_tay23)			robotRunAngle(650,5,-900,0.7);
						else																robotRunAngle(900,5,-900,0.7);
						vTaskDelay(1);
						if(!wantExit()) break;
									}
...
void chuan_bi_gap_thang_do_7(void) {
		int vitri_tay23 = vitri_tay23_phai_laser_GapQua;
	 int vitri_tay41 = vitri_tay23_phai_laser_GapQua;
		kiem_tra_huong_quay_tay_gap_phia_truoc();
