...
void Xuat_Phat_Lay_Vu_Khi_Do(int vu_khi, int goc_ve, int vitri) {					
				...
				for(i=0;i<250;i++)
				{
					...
					{	
						Bam_thanh_laser_trai(41,0,0,vu_khi  ,2,-150,30);
						vTaskDelay(1); 
						if(!wantExit())	break;
					}
				}
				...
				for(i=0;i<250;i++)
				{
					...
					{	
						...
						Bam_thanh_laser_trai(16,0,0,vu_khi ,2,-80,15);
						vTaskDelay(1); 
						if(!wantExit())	break;
					}
				}
				...
				for(i=0;i<100;i++)
				{
					...
					{			
						...
						Bam_thanh_laser_trai(9,0,0,vu_khi,1,-50,15);
						vTaskDelay(1); 
						if(!wantExit())	break;
					}
				}
				...
				robotStop(2);
				chinh_lai_vi_tri_VK_do(vitri, 500, 10, 80, 2);
				robotStop(2);
...
//				
	...
	if (TinHieu_ChuanBi_GapThang == 1) {
			chinh_lai_vi_tri_laser_phai_custom_do(vi_tri_cua_rung_do_3_lazer_phai_tay_41 - 3 , 1000, 5, 65, 7);
	}
	else if (TinHieu_ChuanBi_GapThang == 0) {
			chinh_lai_vi_tri_laser_phai_custom_do(vi_tri_cua_rung_do_3_lazer_phai_tay_41 - 10 , 1000, 5, 65, 7);
	}
...
//				
	...
	if (TinHieu_ChuanBi_GapThang == 1) {
			...
			chinh_lai_vi_tri_laser_phai_custom_do(vi_tri_cua_rung_do_2_lazer_phai_tay_41 - 18 , 1000, 5, 65, 7);
	}
	if (block_pha == 2) {
		SEND_UART (4,1);
			}
...
void Xoay_dau_truoc_cua_do_1(void) {
	...
	if (TinHieu_ChuanBi_GapThang == 1) {
			...
			chinh_lai_vi_tri_laser_phai_custom_do(vi_tri_cua_rung_do_1_lazer_phai_tay_41 - 15 , 1000,5, 60, 7);
	}
				...
				for(i=0;i<250;i++)
					{
						...
						{	
							Bam_thanh_laser_phai(25,900,-900,vitri - 10,2,-150,10);
							vTaskDelay (1);
							if(!wantExit())	break;
						}
				}
...
void Vao_rung_mai_2_do(int vitri)
{
	...
	if (TinHieu_ChuanBi_GapThang == 0) {
			...
			for(i=0;i<1550;i++)
					...
					for(i=0;i<250;i++)
					{
						while(lazeSauValue > 130 || (abs(bientrochansauValue - target_chan_sau) > 200))	
						{	
							Bam_thanh_laser_phai(25,900,-900,vitri ,2,-150,10);
							vTaskDelay (1);
							if(!wantExit())	break;
				...
				for(i=0;i<250;i++)
					{
						while(lazeSauValue > 150)	
						{	
							Bam_thanh_laser_trai(40,900,-900,vitri + 10,2,150,10);
							vTaskDelay (1);
							if(!wantExit())	break;
						}
				}
				...
				for(i=0;i<250;i++)
					{
						while(lazeSauValue > 130)	
						{	
							Bam_thanh_laser_trai(22,900,-900,vitri + 10,2,150,10);
							vTaskDelay (1);
							if(!wantExit())	break;
						}
				}
					...
					for(i=0;i<1550;i++)
							{
									while( CB_Ha_Sau == 1 || CB_Cap_Thanh_Sau == 0)	
								{	
									Bam_thanh_laser_trai(11,900,-900,vitri + 10,2,150,10);
									vTaskDelay (1);
									if(!wantExit())	break;
				...
				for(i=0;i<250;i++)
					{
						while(lazeSauValue > 150)	
						{	
							Bam_thanh_laser_trai(40,900,-900,vitri ,3,150,30);
							vTaskDelay (1);
							if(!wantExit())	break;
						}
				}
				...
				for(i=0;i<250;i++)
					{
						while(lazeSauValue > 130)	
						{	
							Bam_thanh_laser_trai(22,900,-900,vitri ,2,150,30);
							vTaskDelay (1);
							if(!wantExit())	break;
						}
				}
					...
					for(i=0;i<1550;i++)
							{
									while( CB_Ha_Sau == 1 || CB_Cap_Thanh_Sau == 0)	
								{	
									Bam_thanh_laser_trai(11,900,-900,vitri ,2,150,30);
									vTaskDelay (1);
									if(!wantExit())	break;
...
void retry_zone_3_o_1_do(int vitri) {
    ...
    nhom_B = (tay_se_dung_tiep_theo == 2 || tay_se_dung_tiep_theo == 3);
		...
		for(i=0;i<550;i++)	
			...
			for(i=0;i<550;i++)	
			{	
					while(lazeTraiValue > 167)	
					{	
						Bam_laser_truoc(10,0,900, vitri , 1, 150, 10);
						su_dung_chan_cham(512);
						vTaskDelay(1); 
						if(!wantExit())	break;
					}
				}	
...
void chon_o_retry3_do(void) {
    ...
    if  (id_dat_hop <= 3) { 
				...
				RESET_ENCODER(); 
				while(abs(ENCODER_FR()) + abs(ENCODER_FL()) < 4000)	{
				vTaskDelay (1); if(!wantExit())	break;}
				robotStop(0);
				
				chinh_lai_vi_tri_laser_truoc_do(vi_tri_dat_hop_3_tay41_do, 1000, 8, 35);
				su_dung_chan(710);
				robotStop(0);
...
void tu_dong_dat_tang2_do(void) {
        ...
        if (robot_position == 3) {
            ...
            robotStop(0);
            chinh_lai_vi_tri_laser_trai(165, 1000);
        } 
        ...
        if (robot_position == 3) {
            ...
            robotStop(0);
            chinh_lai_vi_tri_laser_trai(165, 1000); /// trg hop tay 3 vi trí 1
        } 
        else if (robot_position == 2) {
...
void tudonghoantoan_san_do(void) {
		...
		Xuat_Phat_Lay_Vu_Khi_Do(152,1450,215);
	// cho rb1 lay vk xong thì vuot rung
		...
		if (hoan_thanh_chay_rung == 1 ) DatKFS_do(156),hoan_thanh_chay_rung = 2 ;
	/// ******* //////
...
void chon_o_nhat_hop_do(void) {
		...
		if (nhat_dat_hop == 1) {
				chinh_lai_vi_tri_laser_phai_zone3_do(188, 200, 4, 45);
			}
		else if (nhat_dat_hop == 2) {
				chinh_lai_vi_tri_laser_phai_zone3_do(219, 200, 4, 45);
			}
		else if (nhat_dat_hop == 3) {
				chinh_lai_vi_tri_laser_phai_zone3_do(250, 200, 4, 45);
			}
		else if (nhat_dat_hop == 4|| nhat_dat_hop == 5 || nhat_dat_hop == 6) {
				chinh_lai_vi_tri_laser_sau_zone3(150, 1000);
				chinh_lai_vi_tri_laser_phai_zone3_do(170, 200, 5, 50);
			}

			robotStop(0);
		if (nhat_dat_hop == 1|| nhat_dat_hop == 2 || nhat_dat_hop == 3) {
				su_dung_chan_thuong(512);
				chinh_lai_vi_tri_laser_sau_zone3(134, 1000);
				robotStop(0);
		...
		// chon be tang 3
				...
				chinh_lai_vi_tri_laser_sau_zone3(150, 1000);
				chinh_lai_vi_tri_laser_phai_zone3_do(170, 200, 4, 50);
				robotStop(0);
...
void nhat_hop_zone3_do (void) {
    ...
    bool thu_lai = false;
		...
		RESET_ENCODER(); 
		while(abs(ENCODER_FR()) + abs(ENCODER_FL()) < 13000)	{vTaskDelay (5); if(!wantExit())	break;}
		robotStop(30);
		...
		// tat trai
		robotRunAngle(900, 28, 0, 0.8);
		for(i=0;i<1550;i++) {
			while( lazePhaiValue > 205)	{	
					vTaskDelay(1);
					if(!wantExit()) break;
								}
						}
		robotStop(30);
		robotRunAngle(1800, 35, 0, 0.8);
		RESET_ENCODER(); 
		while(abs(ENCODER_FR()) + abs(ENCODER_FL()) < 10000)	{vTaskDelay (5); if(!wantExit())	break;}
		robotStop(30);
		
		robotRunAngle(1800, 30, 0, 0.5);
		for(i=0;i<550;i++)	
		{	
				while(lazeSauValue > 105)	
				{	
					vTaskDelay(1); 
					if(!wantExit())	break;
				}
    ...
    // =========================================================
        ...
        Ktra_vtri_kep4();
				...
				RESET_ENCODER(); 
				while(abs(ENCODER_FR()) + abs(ENCODER_FL()) < 2500)	{vTaskDelay (5); if(!wantExit())	break;}
				robotStop(0);
				speed_tay_gat2 = 250;
				target_tay_gat2 = vitri_tay2home; 
				for(i=0; i<50; i++) { 
