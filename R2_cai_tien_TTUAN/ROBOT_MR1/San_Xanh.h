...
void chuan_bi_gap_phai_200_tren_3 (void ) {
	if (da_lay_tay1 == 0 && da_lay_tay2 == 0 && da_lay_tay3 == 0 && da_lay_tay4 == 0) {
			...
			for(i=0;i<1550;i++)
						{
							while(lazeSauValue > 3 || (abs(bientrochansauValue - target_chan_sau) > 200) )	
							{	
								if(!wantExit())	break;
								vTaskDelay (1);
							}
...
//    robotStop(0);
    ...
    if (delta_truoc > SAI_SO_ALLOW && delta_sau > SAI_SO_ALLOW) {
        ...
        if (delta_truoc <= delta_sau) {
            chinh_lai_vi_tri_laser_truoc_do(lz_truoc, 100, 4, 20);
            robotStop(0);
        } 
        else {
            chinh_lai_vi_tri_laser_sau(lz_sau, 100, 4, 20);
...
retry:
    ...
    if (IMUxoay > 16)
    {
        ...
        robotRunAngle(-900, 10, 900, 0.5);
				RESET_ENCODER(); 
				while(abs(ENCODER_FR()) + abs(ENCODER_FL()) < 550)	{vTaskDelay (5); if(!wantExit())	break;}
				robotStop(0);
				vTaskDelay(2000); 

        goto retry;     // quay l?i t? d?u
    }
		vTaskDelay(2000); 
    if (chontay == 1)
    {
