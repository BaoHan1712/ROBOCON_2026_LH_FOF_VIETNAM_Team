...
{	
	...
	robotRunAngle(Angle,31,Robot_Angle,0.5);
...
void chuan_bi_thang_xanh_2(void) {
		...
		if (da_lay_tay1 == 0 && da_lay_tay2 == 0 && da_lay_tay3 == 0 && da_lay_tay4 == 0) {
					...
					{	
					if(lazeTraiValue > vitri_tay41)			robotRunAngle(-600,14,900,0.7);
...
void chuan_bi_gap_thang_xanh_3(void) {
		...
		// quay mam sang tay 2
				...
				while(CB_Cap_Thanh_Sau == 0 )	 {
				if(CB_ROI_4  == 0 && CB_ROI_2 == 0)	robotRunAngle(-1550,20,900,0.7);
...
void qua_trai_len_bac_200_cua_vao(void) {
	...
	if ( TinHieu_ChuanBi_GapThang == 1 ) {
				run_encoder(12000, 1800, 900, 45, 5, 5500, 50);
...
void qua_phai_len_bac_200_cua_vao(void) {
	...
	robotStop(0);
	if ( TinHieu_ChuanBi_GapThang == 1 ) {
				run_encoder(10000, 0, 900, 45, 5, 5000, 100);
				robotStop(0);
			...
			if (chay_ngang_qua_o3_xanh == 0) {
					...
					run_encoder(10000, 0, 900, 45, 5, 5500, 100);
...
//					gap_tay2_duoi();
			...
			// dung tay lan 4
					...
					vTaskDelay(1000);
					Kep_phai_3_ra;
					Gap_tay3_thang(Min_tay_gat1 + 12, 85, 250, 20, 1.1, -900, 900, 4 );
					da_lay_tay4 = 1;
		...
		while(abs(lazeSauValue - vitri) > 0)	{
			...
			speed = custom_smooth_speed(dieuchinh, min_toc, max_toc);
					...
					if  ((lazeSauValue - vitri)> 0 ) {
							robotRunAngle(-900, speed, 900, 0.6);
					}
