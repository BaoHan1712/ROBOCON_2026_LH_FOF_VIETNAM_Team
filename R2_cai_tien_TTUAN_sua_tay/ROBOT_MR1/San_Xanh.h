...
void dat_hop4_2(int do_cao_chan, int goc_tay, int ha_chan, int chontay, int gt_chuanbi);
void dat_hop1_3(int do_cao_chan, int goc_tay, int ha_chan, int chontay, int gt_chuanbi);
...
{				
				...
				for(i=0;i<250;i++)
				{
					while(lazeTruocValue > 157)	
					{	
	
						Bam_thanh_laser_phai(44,-150,0,vu_khi,2,150,10);
				...
				for(i=0;i<250;i++)
				{
					...
					{	
						...
						Bam_thanh_laser_phai(38,-80,0,vu_khi,2,150,10);
				...
				for(i=0;i<250;i++)
				{
					...
					{	
						...
						Bam_thanh_laser_phai(25,0,0,vu_khi,2,100,10);
				...
				for(i=0;i<250;i++)
				{
					while(lazeTruocValue > 134)	
					{	
	
						Bam_thanh_laser_phai(20,0,0,vu_khi,1,80,10);
				...
				for(i=0;i<100;i++)
				{
					while(HT_1 == 0 && HT_2 == 0)	
					{				
						if(lazeTruocValue < 134)	Tay_kep_xuong;
				...
				robotStop(2);
//				
				run_encoder(850, 1800, 0, 38, 6, 400, 10);
				
				robotRun(goc_ve - ((KC_Vu_khi - vu_khi)*5),50);
				robotRotate(-700,-1.4,0);
				...
				while(_robotIMUAngle >-650)	
				{
					vTaskDelay (1); 
					if(!wantExit())	break;
					if(_robotIMUAngle < -650)		target_chan_truoc = 240, target_chan_sau = 240;
				}
				
				target_chan_truoc = 240, target_chan_sau = 240;
				robotStop(0);
 
				robotRunAngle(900,13,-900,0.5);
				
				for(i=0;i<250;i++)
				{
					while(lazeSauValue > 131)	
					{	
						vTaskDelay(1); 
						if(!wantExit())	break;
					}
				...
				vTaskDelay(1000);
				robotStop(0);
				chinh_lai_vi_tri_VK(vitri, 1000, 5, 30, 0);
				robotStop(0);
				robotRunAngle(900,6,-900,0.6);
				vTaskDelay(3000);
				robotStop(0);
...
void Vao_rung_mai_1_xanh(int vitri)
{
	...
	robotSetACC(45, 60); // gia toc nhanh
		...
		for(i=0;i<250;i++)
		{
			...
			{	
			Bam_thanh_laser_phai_cua_vao(62,-900,900,vitri,2,180,30);
				vTaskDelay (1);
				if(!wantExit())	break;
			}
				}
	...
	if (TinHieu_ChuanBi_GapThang == 0) {
			for(i=0;i<250;i++)
				...
				for(i=0;i<250;i++)
				{
					...
					{	
						Bam_thanh_laser_phai_cua_vao(12,-900,900,vitri ,1,150,30);
						if(!wantExit())	break;
					}
				}
				robotStop(0);
...
void Vao_rung_mai_2_xanh(int vitri)
{
	...
	robotSetACC(55, 65); // gia toc nhanh
	
	robotRunAngle(-900,62,900,0.9);
		for(i=0;i<250;i++)
					{
						while(lazeSauValue > 175)	
						{	
			...
			for(i=0;i<1550;i++)
						{
							while(  CB_Cap_Thanh_Sau == 0)	
							{	
								Bam_thanh_laser_phai(12,-900,900,vitri ,1,-150,30);
								if(!wantExit())	break;
								vTaskDelay (1);
							}
						}
				...
				for(i=0;i<1550;i++)
							{
									while( CB_Cap_Thanh_Sau == 0)	
								{	
									Bam_thanh_laser_trai(12,-900,900,vitri ,1,150,30);
									vTaskDelay (1);
									if(!wantExit())	break;
...
void chuan_bi_gap_thang_xanh_4(void) {
		...
		if (da_lay_tay1 == 0 && da_lay_tay2 == 0 && da_lay_tay3 == 0 && da_lay_tay4 == 0) {
				while( lazeSauValue > 130 )	
						...
						else																robotRunAngle(-900,16,900,0.7);
						vTaskDelay(1);
						if(!wantExit()) break;
									}
				...
				for(i=0;i<1550;i++) {
						while( CB_Cap_Thanh_Sau == 0 )	
						{	
						if(lazeTraiValue > vitri_tay41)			robotRunAngle(-450,14,900,0.7);
						else																robotRunAngle(-900,10,900,0.7), Tay2_len;
						vTaskDelay(1);
						if(!wantExit()) break;
									}
							}
				// chay toi roi thi di ngang qua phai
				robotRunAngle(0,14,900,0.7);
				while (lazeTraiValue > vitri_tay41)	 {
					vTaskDelay(1);
					if(!wantExit()) break;
						}
					}
				...
				for(i=0;i<1550;i++) {
						...
						{	
						if(lazeTraiValue > vitri_tay41)			robotRunAngle(-450,14,900,0.7);
						else																robotRunAngle(-900,10,900,0.7), Tay1_len;
						vTaskDelay(1);
						if(!wantExit()) break;
									}
				...
				while( CB_Cap_Thanh_Sau == 0 )	{
						if(CB_ROI_2  == 0 && CB_ROI_4 == 0)	robotRunAngle(-1550,14,900,0.7);
						else																robotRunAngle(-900,10,900,0.7), Tay2_len ;
						vTaskDelay(1);
						if(!wantExit()) break;
						}
...
void chuan_bi_gap_phai_200_tren_3 (void ) {
	if (da_lay_tay1 == 0 && da_lay_tay2 == 0 && da_lay_tay3 == 0 && da_lay_tay4 == 0) {
			...
			Tay2_len;	
			chinh_lai_vi_tri_laser_trai_custom(vitri_tay41_laser_trai_GapQua , 200);
			robotRunAngle(0,12,900,0.5);
			vTaskDelay(2500);
...
void chuan_bi_gap_trai_200_tren_1 (void ) {
	if (da_lay_tay1 == 0 && da_lay_tay2 == 0 && da_lay_tay3 == 0 && da_lay_tay4 == 0) {
			...
			for(i=0;i<1550;i++) {
						while( lazeSauValue > 132 )	
						{	
						if(lazePhaiValue > vitri_tay23_phai_laser_GapQua)			robotRunAngle(-1550,22,900,0.7);
...
void chuan_bi_gap_trai_200_duoi_5 (void ) {
	...
	if (da_lay_tay1 == 0 && da_lay_tay2 == 0 && da_lay_tay3 == 0 && da_lay_tay4 == 0) {
				// chuan bi xoay truoc sang trai
					ha_sat_xuong();
					while(lazeSauValue > 132 )	 {
					if(CB_ROI_2  == 0 && CB_ROI_4 == 0)	robotRunAngle(-1550,25,900,0.7);
					else																robotRunAngle(-900,15,900,0.7);
					vTaskDelay(1);
					if(!wantExit()) break;
								}
					
					robotRunAngle(1800,25,900,0.5);
					for(i=0;i<150;i++)
					{
						while(CB_ROI_2 == 0 )	
						{	
...
void chuan_bi_gap_thang_xanh_7(void) {
	...
	// chinh lai vi tri cho tay gap
		...
		robotRunAngle(-900,8,900,0.7);
			for(i=0;i<500;i++)	
					{	
			while( CB_gap_qua_duoi == 0)	{vTaskDelay(1); if(!wantExit())	break;}
					}
		robotStop(0);	
}
...
void chuan_bi_gap_phai_200_tren_7_10 (void ) {
	...
	if (da_lay_tay1 == 0 && da_lay_tay2 == 0 && da_lay_tay3 == 0 && da_lay_tay4 == 0) {
			...
			robotRunAngle(100,16,900,0.5);
				for(i=0;i<150;i++)
					{
						while(lazeTraiValue > vitri_tay41_laser_trai_GapQua + 5)	
						{	
							vTaskDelay (1);
							if(!wantExit())	break;
						}
			...
			robotRunAngle(-650,14,900,0.8);
			RESET_ENCODER(); 
			while(abs(ENCODER_FR()) + abs(ENCODER_FL()) < 1800)	{vTaskDelay (1); if(!wantExit())	break;}
			Tay2_len;
			robotRunAngle(0,16,900,0.5);
				for(i=0;i<150;i++)
					{
						while(lazeTraiValue > vitri_tay41_laser_trai_GapQua + 5)	
						{	
...
void chuan_bi_gap_thang_xanh_9 (void) {
		...
		if (da_lay_tay1 == 0 && da_lay_tay2 == 0 && da_lay_tay3 == 0 && da_lay_tay4 == 0) {
				...
				while (CB_ROI_3  == 0 )	 {
					...
					// chay toi roi thi di ngang qua trai
				robotRunAngle(1800,14,900,0.7);
				while (lazePhaiValue > vitri_tay23)	 {
					vTaskDelay(1);
					if(!wantExit()) break;
						}
			}
...
void chuan_bi_gap_trai_200_tren (void ) {
	if (da_lay_tay1 == 0 && da_lay_tay2 == 0 && da_lay_tay3 == 0 && da_lay_tay4 == 0) {
			...
			robotRunAngle(-1800,16,900,0.6);
				for(i=0;i<150;i++)
					{
				while(lazePhaiValue > vitri_tay23_phai_laser_GapQua + 5 )	
						{	
							vTaskDelay (1);
							if(!wantExit())	break;
						}
...
void qua_phai_len_bac_200_6_xanh (void) {
		...
		int di_cheo_41 = 0;
		int di_cheo_23 = 0;
		if (TinHieu_QuaPhai == 1 && (bientrochansauValue >= 300) && (CB_ROI_2 == 1)) { // dag dung mep trai de lay thang
				time_41 = 8000;
				time_23 = 8000;
				di_cheo_41 = 450;
				di_cheo_23 = 450;
		}
		else if (TinHieu_QuaPhai == 1 && bientrochansauValue >= 300 && (lazeSauValue < 130) && (CB_ROI_2 == 0))  { // dung truoc
				time_41 = 6000;
				time_23 = 6000;
				di_cheo_41 = 1250;
				di_cheo_23 = 1250;
		}
		else if (TinHieu_QuaPhai == 1 && (bientrochansauValue <= 300)&& (CB_ROI_2 == 1) ) { // dag dung mep trai lay trai
				time_41 = 4500;
				time_23 = 4500;
				di_cheo_41 = 450;
				di_cheo_23 = 250;
		}
		else if (TinHieu_QuaPhai == 1 && (bientrochansauValue <= 300 || bientrochansauValue >= 300)&& (CB_ROI_2 == 0) ) { // dag dung giua o
				time_41 = 500;
				time_23 = 500;
				di_cheo_41 = 450;
				di_cheo_23 = 250;
...
void qua_phai_xuong_bac_200_9_xanh (void) {
		...
		else if (TinHieu_QuaPhai == 1 && CB_Ha_Sau == 0 && (CB_ROI_3 == 1) ) { //dag dung o mep phai
				time_41 = 4000;
				time_23 = 4000;
				di_cheo_41 = -1450;
				di_cheo_23 = 1450;
		}
		else if (TinHieu_QuaPhai == 1 && CB_Ha_Sau == 0 && (CB_ROI_3 == 0) ) { //dag o giua o
				time_41 = 1000;
				time_23 = 1000;
				di_cheo_41 = -1450;
				di_cheo_23 = 450;
		}
		else if (TinHieu_QuaPhai == 1 && CB_Ha_Sau == 0 && (CB_ROI_2 == 1) ) { //dag o mep trai
				time_41 = 4000;
				time_23 = 4000;
				di_cheo_41 = 450;
				di_cheo_23 = -450;
...
void qua_phai_xuong_bac_200_12_xanh (void) {
		...
		if (TinHieu_QuaPhai == 1 && CB_Ha_Sau == 1 && (bientrochansauValue <= 300)) { // th vua gap trc xong
				...
				else if (TinHieu_QuaPhai == 1 && CB_Ha_Sau == 0 && (CB_ROI_3 == 1) && (bientrochansauValue <= 300) ) { //dag dung o mep phai
				time_41 = 4000;
				time_23 = 4000;
				di_cheo_41 = -1450;
				di_cheo_23 = 1450;
		}
		else if (TinHieu_QuaPhai == 1 && CB_Ha_Sau == 0 && (CB_ROI_3 == 0) && (bientrochansauValue <= 300) ) { //dag o giua o
				time_41 = 1000;
				time_23 = 1000;
				di_cheo_41 = -1450;
				di_cheo_23 = 450;
		}
		else if (TinHieu_QuaPhai == 1 && CB_Ha_Sau == 0 && (CB_ROI_2 == 1) && (bientrochansauValue <= 300) ) { //dag o mep trai
				time_41 = 4000;
				time_23 = 4000;
				di_cheo_41 = 450;
				di_cheo_23 = -450;
...
void dung_cho_khoi_r1_duoi_MD (int Angle, int Robot_Angle) {
			...
			robotRunAngle(Angle,20,Robot_Angle,1);
...
void kiem_tra_qua_tay_nao(void)
{
    ...
    // Uu tiên tuyet doi: Tay 1 2 co qua
    if (tay_1_co_qua || tay_2_co_qua)
    {
        dang_o_nhom = 1;
        target_xoay_tay = vi_tri_dat_hop_1_2;
    }
    else if (tay_3_co_qua || tay_4_co_qua)
    {
        dang_o_nhom = 2;
  ...
  target_xoay_tay = vi_tri_dat_hop_1_2;
	...
	for(i=0;i<150;i++)		
	{
		...
		if (vi_tri_chay_ngang_xanh == 10) {
			robotSetACC(50, 55); // gia toc
			chinh_lai_vi_tri_laser_phai_custom(182, 1000, 12, 75, 2);
			robotSetACC(120, 85); // gia toc
	}
	else if (vi_tri_chay_ngang_xanh == 11) { 
			robotSetACC(60, 65); // gia toc
			chinh_lai_vi_tri_laser_phai_custom(182, 1000, 11, 75, 3);
			robotSetACC(120, 85); // gia toc
	}
	else if ( vi_tri_chay_ngang_xanh == 12) { 
			robotSetACC(65, 75); // gia toc nhanh
			chinh_lai_vi_tri_laser_phai_custom(182, 1000, 10, 75, 3);
			robotSetACC(120, 85); // gia toc
	}
...
///// da di len roi chay ngang qua
		for(i=0;i<550;i++)	
		{	
			while(lazePhaiValue < 240)	
			{	
				Bam_laser_sau(65,0,900, 136, 2, -100, 10);
				vTaskDelay(1); 
				if(!wantExit())	break;
			}
		}
		
		///// chay ngang	check r1 tu khoang 240. den 255

		for(i=0;i<550;i++)	
		{	
			while(lazePhaiValue < 255)	
			{	
				Bam_laser_sau(60,0,900, 136, 2, -100, 10);
...
////////// =================================================
		...
		if (phat_hien_r1_v3 == 0) { 
				robotRunAngle(20,65,900,0.6);
				for(i=0;i<550;i++)	
				{	
		...
		/////==========================////////////////////////
		else if (phat_hien_r1_v3 == 1) { 
				led_bao_hieu_on;
				Tay_kep_mo;
				run_encoder(5000, 0, 900, 45, 5, 4000, 10);
				robotStop(0);
				...
				robotRunAngle(500,60,900,0.6);
				for(i=0;i<550;i++)	
				{	
					while(lazeTruocValue > 188)	
					{	
						vTaskDelay(1); 
						if(!wantExit())	break;
					}
...
//	**************** che do cay diem  ******************
	if (mode_cay_diem == 1 ) {
			...
			{	
					...
					{	
						Bam_laser_sau(50,0,900, vitri - 20, 3, -80, 10);
						vTaskDelay(1); 
						if(!wantExit())	break;
					}
			}
	...
	if (mode_cay_diem == 2 ) {
		...
		for(i=0;i<550;i++)	
			{	
					...
					{	
						Bam_laser_sau(25,0,900, vitri , 2, -150, 10);
						vTaskDelay(1); 
						if(!wantExit())	break;
					}
			}	
			...
			{	
					...
					{	
						Bam_laser_sau(10,0,900, vitri  , 2, -150, 10);
...
void dat_hop_vao_o (void) {
		if (tin_hieu_dat_hop_1_2 == 1 ) {
				dat_qua_tay_1_2_xanh();
				tin_hieu_dat_hop_1_2 = 0;
		}
		else if (tin_hieu_dat_hop_3_4 == 1 ) {
...
void dat_hop_vao_o_tang2 (void) {
		if (tin_hieu_dat_hop_3_4 == 1 ) {
				...
				if ((CB_kep_1_2 == 0  || CB_kep_1_1 == 0 ) || check_hop_1 == 0) {
						...
						for(i=0; i<550; i++) 
						{ 
						while(abs(bientrodaytay1Value - target_tay_gat1) > 4) { vTaskDelay(1); if(!wantExit()) break; }
						}
						dat_hop1_3(490, 560, 5, 1, 475);
						vTaskDelay(2000); 
...
void chuan_bi_dat_hop_tang3 (void) {
		if (tin_hieu_dat_hop_3_4 == 1 && tay_4_co_qua == 1 ) {
				speed_tay_gat1 = 120;
        target_tay_gat1 = vitri_tay1home + 125;
				Tay2_len;
				for(i=0;i<50;i++)	{ 
				...
				speed_tay_gat2 = 120;
        target_tay_gat2 = vitri_tay2home - 125;
				Tay1_len;
				for(i=0;i<50;i++)	{ 
...
void kiem_tra_so_qua_tren_tay(void) {
    ...
    // Ki?m tra tay 2
    if (check_hop_2 == 0 || (CB_kep_2_1 == 0 || CB_kep_2_2 == 0 ) ) {
        tay_2_co_qua = 1;
				da_dat_tay_2 = false;
        dem++;
    } else {
...
void be_len_dat_xanh_4_qua (void) {
    ...
    if (nhay_den_lan_1 == 1 ) {
        su_dung_chan(350);
        for(i=0; i<150; i++) {
            while(abs(bientrochansauValue - target_chan_sau) > 150) {
                vTaskDelay(1); 
                if(!wantExit()) break;
            }
        }
...
//		run_encoder(3500, 1800, 900, 35, 10, 1500, 50);
		for(i=0;i<550;i++)	
		{	
				while(lazeTraiValue < 176)	
				{	
					Bam_laser_truoc(20,1800,900, 158 , 2, -120, 10);
					if (lazeTraiValue > 166) {
						Kep_phai_1_vao;
						Kep_phai_2_vao;
						Kep_phai_4_vao;
						Kep_phai_3_vao;
					...
					if (lazeTraiValue > 173) { 
							Tay1_xuong;
							Tay2_xuong;
					}
					if (lazeTraiValue > 174) { 
...
void tudonghoantoan(void) {
		...
		Xuat_Phat_Lay_Vu_Khi_Xanh(164, -1600, 199);
    mo_vk_san_xanh();
    ...
    nhom_B = (tay_se_dung_tiep_theo == 2 || tay_se_dung_tiep_theo == 3);
		...
		for(i=0;i<550;i++)	
			{	
					while(lazeTraiValue > 175)	
					{	
						Bam_laser_sau(25,0,900, vitri , 3, -250, 10);
						vTaskDelay(5); 
						if(!wantExit())	break;
					}
			}
...
retry:
    t = 0;
		...
		for(i=0; i<550; i++) 
			{ 
		while(abs(bientrodaytay2Value - target_tay_gat2) > 4) { vTaskDelay(1); if(!wantExit()) break; }
			}
		vTaskDelay(1500);
...
void chinh_lai_vi_tri_laser_sau_zone3(int vitri, int gia_tri_lap)
{
	...
	int dieuchinh;
		...
		while(abs(lazeSauValue - vitri) > 0)	{
			...
			speed = custom_smooth_speed(dieuchinh, 6, 50);
...
void chon_o_nhat_hop_xanh(void) {
		...
		if (nhat_dat_hop == 1 ) {
				chinh_lai_vi_tri_laser_phai_zone3_do(186, 200, 7, 55);
			}
		else if (nhat_dat_hop == 2 ) {
				chinh_lai_vi_tri_laser_phai_zone3_do(217, 200, 7, 55);
			}
		else if (nhat_dat_hop == 3 ) {
				chinh_lai_vi_tri_laser_phai_zone3_do(248, 200, 7, 55);
			}
		else if (nhat_dat_hop == 4|| nhat_dat_hop == 5 || nhat_dat_hop == 6) {
				chinh_lai_vi_tri_laser_sau_zone3(165, 1000);
				chinh_lai_vi_tri_laser_phai_zone3_do(187, 200, 8, 55);
			}
		...
		if (nhat_dat_hop == 1|| nhat_dat_hop == 2 || nhat_dat_hop == 3) {
			...
			if (da_lay_tay1 == 0 && da_lay_tay2 == 0) {
				...
				dat_hop4_2_retry(490, 330, 5, 4, -900, 900, 0, 415);
				vTaskDelay(2000); 
				speed_tay_gat2 = 250;
				target_tay_gat2 = vitri_tay2home ;
				for(i=0; i<550; i++) 
...
//				chinh_lai_vi_tri_laser_sau_zone3(165, 1000);
				robotStop(0);
				chinh_lai_vi_tri_laser_phai_zone3_do(187, 200, 7, 55);
				robotStop(0);
			
//				robotRotate(-890,1.1,0);
//				while(robotFixAngle()){
//					vTaskDelay (1); 
//					if(!wantExit())	break;
//					}
				robotRotateQuick(-850, 1.6, 0, 0.3, 300);
				robotStop(0);
				su_dung_chan(730);
				robotStop(0);
				quyet_dinh_dat_hop_tang3();
				robotStop(0);
				robotRotate(10, 0.8, 0);
        while(robotFixAngle()) {
            vTaskDelay (1); 
            if(!wantExit()) break;
        }
...
void nhat_hop_zone3_xanh (void) {
    ...
    su_dung_chan_thuong(400);
		...
		run_encoder(19000, 1800, 0, 80, 10, 6500, 50);
    ...
    // chi tracking hop 
    if (da_lay_tay1 == 0 && da_lay_tay2 == 0) {
			ha_tay_4_k_xlang(145, 75, 250, 20, 1, -900, 900, 7);
			vTaskDelay(1500);
			Kep_phai_4_vao;
			vTaskDelay(3000);
...
//    
    ...
    for (lap = 0; lap < 8; lap++) {
      // chuan bi lay lan tiep theo
				...
				Tay2_xuong;
				robotStop(0);
				run_encoder(3000, 0, 0, 45, 10, 1500, 10);
				robotStop(0);
				
////				su_dung_chan_thuong(300);
				Nang_thuong();
				target_chan_truoc = 304 ;
				target_chan_sau = 300;
				robotStop(0);
							
				speed_tay_xoay = 220;
				target_xoay_tay = vi_tri_tay_43;
				chinh_lai_vi_tri_laser_phai_zone3_do(160, 500, 5, 45);
        chinh_lai_vi_tri_laser_sau_zone3(255, 1000);
        robotStop(0);
