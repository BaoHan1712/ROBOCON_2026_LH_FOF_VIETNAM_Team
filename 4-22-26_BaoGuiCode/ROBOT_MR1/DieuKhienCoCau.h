...
int	Min_tay_gat1 = 96, Max_tay_gat1 = 784, target_tay_gat1;
...
void chanTruoc(int tocDo){
	if(abs(tocDo) <= 2){
		if(tocDo == 2) tocDo = 2;
		else tocDo = 0;
		mor_lift_front = abs(tocDo);
	}
	else if(tocDo > 0) {
		mor_lift_front_up;
		mor_lift_front = abs(tocDo);
...
void Chan_truoc()
{
	float derivative, maxStep, delta;
	
	int hanhTrinh = target_chan_truoc;
	int tocDo = speed_chan_truoc;
	
    static float tocDoTruoc = 0;

    // ===== PID STATE =====
    static float integral = 0;
    static float errorTruoc = 0;

    // ===== THAM SO PID =====
    float Kp = 4.0f;
    float Ki = 0.06f;
    float Kd = 50.0f;   // ?? h? s? D (b?t d?u nh?)

    float error, tocDoHienTai;
		// ===== GIOI HAN TOCDO =====	
		//-250 < tocDo < 250
			if (tocDo > 254) tocDo = 254;
			else if (tocDo < -254) tocDo = -254;
			// ===== GIOI HAN HANH TRINH =====
	//    if (hanhTrinh < -480) hanhTrinh = -480;
	//    else if (hanhTrinh > 0) hanhTrinh = 0;
	
    // ===== TINH ERROR =====
    error = hanhTrinh - bientrochantruocValue;
    ...
    // ===== DAO HAM (D) =====
    derivative = error - errorTruoc;
    errorTruoc = error;

    // ===== PID =====
    tocDoHienTai = Kp * error
                 + Ki * integral
                 + Kd * derivative;

    // ===== GIOI HAN TOC DO =====
    if (tocDoHienTai > tocDo)
        tocDoHienTai = tocDo;
    else if (tocDoHienTai < -tocDo)
        tocDoHienTai = -tocDo;
		//0 tha troi, 1 khong co, 2 la khoa, xung tu 3 - 255
		if (tocDoHienTai > 0){
			if(tocDoHienTai <= 3) tocDoHienTai = 3;
		}
		else {
			if(tocDoHienTai >= -3) tocDoHienTai = -3;
		}
		
		if(abs(error) <= 1) tocDoHienTai = 2;
    // ===== GIOI HAN GIA TOC =====
    maxStep = 2.0f;
    delta = tocDoHienTai - tocDoTruoc;

    if (delta > maxStep)
        tocDoHienTai = tocDoTruoc + maxStep;
    else if (delta < -maxStep)
        tocDoHienTai = tocDoTruoc - maxStep;

    tocDoTruoc = tocDoHienTai;

    // ===== DIEU KHIEN MOTOR =====
    chanTruoc(tocDoHienTai);
}
// ===== PID Variables cho Chan Truoc =====
//float pid_kp_chan_truoc = 0.5f;
//float pid_ki_chan_truoc = 0.04f;
//float pid_kd_chan_truoc = 1.5f;
...
float pid_ki_chan_sau = 0.04f;
float pid_kd_chan_sau = 1.25f;
...
void Chan_sau()
{
    ...
    if (abs(error) > 4)
    {
        ...
        // === Tính speed tu PID output ===
        speed_temp = (uint16_t)abs(pid_output_chan_sau) * 3;

        if (speed_temp > speed_chan_sau)
            speed_temp = speed_chan_sau;

        // Gioi han duoi motor không bi chet
        if (speed_temp < 20)
            speed_temp = 20;
    ...
    {
        // Ðã den target thi giu vi trí, reset integral
...
void Nang_nhanh	(void) {
	speed_chan_sau = 210;
	speed_chan_truoc = 212;
}

void Nang_cuc_nhanh	(void) {
	speed_chan_sau = 245;
	speed_chan_truoc = 247;
}
...
//        
//        for(i = 0; i < 50; i++) { 
//            while(abs(bientrodaytay1Value - (Max_tay_gat1 - 45)) > 8) {
//                speed_tay_gat1 = 250;
