...
//					if(O)	target_xoay_tay = Min_xoay_tay,tay_xoay = 10;
//						if(O) 					Tay_gap_kfs_len();
//						else if(SQUARE) Tay_gap_kfs_xuong();
//						else						Tay_gap_kfs_dung();
				if(O)	Kep = 250, Nang_Ha_Kep = 250;
//				if(TRIANGLE)	XI_LANH_NANG_KEP;
//				if(O)	XI_LANH_KEP_MO;
//				if(X)	XI_LANH_KEP_DONG;
				
//					if(SQUARE)	target_xoay_tay = Min_xoay_tay;
//					if(O)				target_xoay_tay = Max_xoay_tay;
//					if(TRIANGLE)				target_xoay_tay = 520;
//					speed_chan_sau =180;
//					speed_chan_truoc = 240;
					if(SQUARE)			target_chan_truoc = 590,target_chan_sau =590;
...
//				if(O && L2 && !R2 && Vi_tri == 1)								Vao_rung_mai_3();
					if(O && L2 && !R2 && Vi_tri == 1)								Run_All_Blocks_From_Queue();
//				if(O && L2 && !R2 && Vi_tri == 1)									Vao_rung_mai_1();
//				if(SQUARE && L2 && !R2 && Vi_tri == 1)					Vao_rung_mai_1();
				
//				if(O && L2 && !R2 )								Run_All_Blocks_From_Queue();
//				if(SQUARE && L2 && !R2)						Xuong_bac_200();
//				if (OPTIONS) 											chaytudong_esp_now();
				
				
			}
			
//			if(SQUARE && !R1 &&!L2 && !R2)	Leo_bac_200_trc();
//			if(TRIANGLE && !R1 && !L2 && !R2)Leo_bac_400_trc();
//			if(SQUARE  && R1 && !L2 && !R2)	Leo_bac_200_sau();
//			if(TRIANGLE && R1 && !L2 && !R2)Leo_bac_400_sau();
//			if(X && !R1 && !L2 && !R2)			Xuong_bac_200_trc();
//			if(O && !R1 && !L2 && !R2)  		Xuong_bac_400_trc();
//			if(X && R1 && !L2 && !R2)				Xuong_bac_200_sau();
//			if(O && R1 && !L2 && !R2)				Xuong_bac_400_sau();
