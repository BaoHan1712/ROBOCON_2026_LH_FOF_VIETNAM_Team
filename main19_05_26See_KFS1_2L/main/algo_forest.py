...
# =====================================================================
    ...
    def solve_auto_targets(self):
        ...
        def get_sort_key(sol):
            ...
            return (
                pick_penalty,         # 👑 ƯU TIÊN 1: Vẫn phải ưu tiên lấy đủ mục tiêu (KPI) sếp giao!
                sol["shifts"],        # 🥈 ƯU TIÊN 2: LƯỜI RẼ NHẤT! Đường thẳng tắp (0 rẽ) sẽ luôn thắng đường lạng lách (1 rẽ), cho dù có đâm 2 cục R1!
                sol["r1_stepped"],    # 🥉 ƯU TIÊN 3: Nếu 2 đường CÙNG đi thẳng, hoặc CÙNG rẽ 1 lần, thì lúc này mới so xem thằng nào ÍT R1 hơn để đi.
                -sol["picked_count"], # 🏅 ƯU TIÊN 4: Nếu kẹt đường buộc phải rớt KPI, chọn đường vớt vát nhiều cục nhất
                sol["cost"],          # 🏅 ƯU TIÊN 5: So sánh chi phí phụ (chạy gầm xa, v.v...)
                dist_to_exit          # 🏅 ƯU TIÊN 6: Ép góc thoát dạt ra mép sân
            )
            ...
            for i in range(len(segment_path) - 1):
                ...
                if 0 <= curr_r < ROWS and 0 <= next_r < ROWS:
                    ...
                    if cell_next["content"] and cell_next["content"]["number"] == 1:
                        ...
                        if r1_encounter_count == 2:
                            print(f"  │  ├─ [KHỐI 1 LẦN 2] Gửi: (2, 2, 4, 5, {step_id}) — Mở Cam CHECK R1_L2 Né Cản")
                            time.sleep(0.05)
                            ser.write(build_packet(2, 2, 4, 5, step_id))
                            time.sleep(0.05)
                        else:
                            ...
                            time.sleep(0.05)
                            # =========================================================
                            # 🔥 ĐÃ CHỈNH CHUẨN ĐỊNH DẠNG: Mở Cam check R1 cục đầu tiên trên sa bàn
                            # =========================================================
                            print(f"  │  ├─ [GỌI CAM L2] Bắn bồi lệnh mở Cam né cản: (2, 2, 4, 5, {step_id})")
                            ser.write(build_packet(2, 2, 4, 5, step_id))
                            time.sleep(0.05)
