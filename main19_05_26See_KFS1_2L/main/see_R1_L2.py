...
from gui_tkinter import set_state, STATE_IDLE, STATE_SEE_R1_L2
# ================= POLYGON =================
points = np.array([
    (28,331),
    (623,331),
...
# ================= HẰNG SỐ =================
DEPTH_THRESHOLD = 380  # mm - ngưỡng phát hiện vật cản
DETECT_TIME = 0.5  # giây - thời gian giữ để confirm
MIN_POINTS = 20  # số điểm tối thiểu để coi là có vật cản

# ================= MAIN =================
def detect_r1_l2_loop(use_state=True):
    """
    ...
    while True:
        try:
            ...
            time.sleep(0.5) # Chờ 5s để USB kịp nhận lại thiết bị
    ...
    try:
        while True:
            # ===== state change → thoát =====
            # Đổi STATE_SEE_R1 thành STATE_SEE_R1_L2
            if use_state and set_state["value"] != STATE_SEE_R1_L2:
                break
            ...
            try:
                ...
                if not has_object:  # KHÔNG có vật cản
                    ...
                    if duration >= DETECT_TIME and not sent_clear:
                        ...
                        start_packet = build_packet(2, 2, 4, 6, 10)
